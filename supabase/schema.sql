...
-- ------------------------------------------------------------
-- Auto-create a profile the instant an auth.users row is created —
-- fires regardless of email-confirmation state, and runs as the
-- table owner (security definer), so it isn't blocked by RLS the
-- way a client-side insert from the (unauthenticated-until-confirmed)
-- new user would be. `name`/`username` come from the metadata passed
-- to `supabase.auth.signUp({ options: { data: {...} } })`.
-- ------------------------------------------------------------
create or replace function handle_new_user()
returns trigger as $$
begin
  insert into public.profiles (id, name, username, email, role, balance)
  values (
    new.id,
